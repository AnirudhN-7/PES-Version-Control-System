...
int object_write(ObjectType type, const void *data, size_t len, ObjectID *id_out) {
    ...
    // +1 to include the '\0' that terminates the header
    size_t full_len = (size_t)header_len + 1 + len;

    // 2. Assemble full object: header + '\0' + data
    uint8_t *full = malloc(full_len);
    if (!full) return -1;
    memcpy(full, header, (size_t)header_len + 1);   // copies the '\0' too
    ...
    // 5. Ensure shard directory exists  (.pes/objects/XX/)
    char path[512];
    ...
    mkdir(shard_dir, 0755);   // ignore EEXIST
    ...
    // 7. fsync the temp file so data reaches disk before rename
...
int object_read(const ObjectID *id, ObjectType *type_out, void **data_out, size_t *len_out) {
    ...
    // 5. Parse type string and size from header
    char type_str[16] = {0};
    size_t stored_size = 0;
    if (sscanf((char *)buf, "%15s %zu", type_str, &stored_size) != 2) {
        free(buf); return -1;
    }
