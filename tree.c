...
static int write_tree_level(IndexEntry *entries, int count, ObjectID *id_out) {
    ...
    while (i < count) {
        ...
        if (!slash) {
            ...
            // Build sub-array with prefix stripped
            int sub_count = j - i;
            IndexEntry *sub = malloc((size_t)sub_count * sizeof(IndexEntry));
            if (!sub) return -1;
            for (int k = 0; k < sub_count; k++) {
                sub[k] = entries[i + k];
                const char *rest = entries[i + k].path + dir_len + 1;
                strncpy(sub[k].path, rest, sizeof(sub[k].path) - 1);
