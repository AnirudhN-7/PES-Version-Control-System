...
int index_load(Index *index) {
   ...
   index->count = 0;
    ...
    // Each line: "<mode-octal> <64-hex-hash> <mtime> <size> <path>"
...
int index_add(Index *index, const char *path) {
 ...
 // 1. Read file contents
    ...
    uint8_t *contents = malloc((size_t)file_sz + 1);
    if (!contents) { fclose(f); return -1; }
    size_t nread = fread(contents, 1, (size_t)file_sz, f);
    fclose(f);
    if (nread != (size_t)file_sz) { free(contents); return -1; }
